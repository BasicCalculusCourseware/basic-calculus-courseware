...
import { STAtoms, useSetModal } from '.';

// MAIN-COMPONENT
export default function ViewerModal() {
    // RECOIL STATES
    const [selected, setSelected] = useRecoilState(STAtoms.selected);
    // RECOIL VALUES
    const { viewer: isModalOpen } = useRecoilValue(STAtoms.modals);
    const assessment = useRecoilValue(STAtoms.assessment);
    // RECOIL CUSTOM HOOKS
    const setModal = useSetModal();
    // UTILS
    const handleClose = () => {
        setSelected(initialStates.submittedAssessment);
        setModal({ viewer: false });
    };
    // RENDER
    return (
